...
for (i in 1:length(NeuronG1)) {
  ...
  for (j in 1:37){
    ...
    h1response=sum(PNtoh1$w1.weight/PNtoh1$h1total)*PN4trial[j+1]
    df_i[j,]=t(h1response)
  }
...
for (i in 1:length(NeuronG3)) {
  ...
  if (length(Pre_path6) == 0) {
    ...
    for (j in 1:length(Pre_path6)){
      ...
      h2response=NeuronG1_path4_Release[,3:102][NeuronG1_path4_Release$NG1ID==Pre_path6[j],]*(SinglePre[1,]$w2.weight/SinglePre[1,]$h2Atotal)
      Path6[j,]=h2response
    }
  }
...
#  MBONsign=MBON_store[i,3:102]*MBON_list_all$sign[grep(MBON_store$G1N_type[i],MBON_list_all$MBONname)]
#  MBONsignAll=rbind(MBONsignAll,MBONsign)
#}
#NeuronG3_path6_Release[3:102][grep("MBON",NeuronG3_path6_Release$G1N_type),]=MBONsignAll
...
write.table(ALDANMatrix, "NN_byBlock_0_100_4trials_wEI_1_DbyPN_MBON.csv", col.names=FALSE, sep = ",")
NN_byBlock0_100_4trials_wEI_1_DbyPN_MBON=ALDANMatrix
