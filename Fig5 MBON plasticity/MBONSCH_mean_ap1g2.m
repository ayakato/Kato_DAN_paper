...
triggertiming=[18,47,74,102,130,157,185,213,240,268];
...
triggertiming=[18,47,74,102,130];%,157]%,185,213,240,268]%+8;
...
WR=zeros(5,6,NumBrain);
for i=1:8
    WR=WR+AllRes([triggertiming+i+8],:,:)-AllRes([triggertiming-1],:,:);
end       
...
for i=1:8
    ...
    errorbar(WRM(:,1),WRE(:,1),'-s','Color',[1,0,1],"LineWidth",2); 
    errorbar(WRM(:,3),WRE(:,3),'-s','Color',[1,0.1,0.6],"LineWidth",2); 
    errorbar(WRM(:,5),WRE(:,5),'-s','Color',[0.8,0.4,0.8],"LineWidth",2); 
...
MBON=[squeeze(WRD(1:10,1,:))';squeeze(WRD(1:10,3,:))';squeeze(WRD(1:10,5,:))'];
...
MBON=[squeeze(WRD(1:10,2,:))';squeeze(WRD(1:10,4,:))';squeeze(WRD(1:10,6,:))'];
